...
for (i = 0; i < books.length; i++) {
    if (books[i].books - books[i].books > 0) {
      books++;
    }
    return books
...
//     ];
    function getBooksByAuthor() {
        ...
        for (let i = 0; i < book.length; i++) {
          let title = bookname[i].bookname.reduce((total,next) => total + next) /bookname[i].book.length;
          if(title=bookname) {
            // book.push(bookname[i].name)
            book +=title
          }
          return title
...
function checkoutBook(booksTitle){
    empty = []
